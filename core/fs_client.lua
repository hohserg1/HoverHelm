...
function fs_client.fs_connect()
    bios.sendAwait("fs_connect",bios.name)

    local fs_address=vcomponent.uuid()

    local proxy = {
		__index=function(t,k)
            return function(...)
                if k=="list" then --recieve list of files in unpacked form
                    local r=table.pack(bios.sendAwait("fs_component_invoke", k,...))
                    return r
                else
                    return bios.sendAwait("fs_component_invoke", k,...)
                end
			end
		end
    }
