...
function TComponent:draw()
	...
	if brd then
		gpu.set(
			self.X,
			self.Y,
			brd[1] .. string.rep(brd[2], self.W - 2) .. brd[3]
		)
		...
		gpu.set(
			self.X,
			self.Y + self.H - 1,
			brd[4] .. string.rep(brd[2], self.W - 2) .. brd[6]
		)
	else
...
function TComponent:destruct()
	if self.parent then
		for i = 1, #self.parent.elements do
			if self.parent.elements[i] == self then
...
function forms.activeForm()
	return activeForm
end

----------Визуальные компоненты---------
------------------Form------------------
local TForm = setmetatable(
	{ type = function()
		return "Form"
	end },
	TComponent
...
function TButton:touch(x, y, btn, user)
	if self.onClick and btn == 0 then
		self:onClick(user)
	end
end
...
function TButton:paint()
	gpu.set(
		self.X + (self.W - len(self.caption)) / 2,
		self.Y + (self.H - 1) / 2,
		self.caption
	)
end
...
local function editText(self, text, left, top, W, H,user)
	...
	local function setCursor(nx, ny)
		...
		if posX > len(text[posY]) + 1 then
			posX = len(text[posY]) + 1
		end
		if posX < 1 then
			posX = 1
		end
		local redraw = false
		if posY <= scrollY then
			scrollY = posY - 1
			redraw = true
		end
		if posY > scrollY + H then
			scrollY = posY - H
			redraw = true
		end
		if posX <= scrollX then
			scrollX = posX - 1
			redraw = true
		end
		if posX > scrollX + W then
			scrollX = posX - W
			redraw = true
		end
		if redraw then
			...
			term.setCursor(left + posX - scrollX - 1, top + posY - scrollY - 1)
		end
	end
	...
	local function insert(value)
		...
		text[posY] =
			sub(text[posY], 1, posX - 1) .. value .. sub(text[posY], posX)
		writeLine(posY)
	...
	keys[211] = function()
		...
		if posX <= len(text[posY]) then
			text[posY] =
				sub(text[posY], 1, posX - 1) .. sub(text[posY], posX + 1)
			writeLine(posY)
		else
			if posY < #text then
	...
	if type(text) == "table" then
		...
		keys[28] = function()
			...
			table.insert(
				text,
				posY + 1,
				string.rep(" ", n) .. sub(text[posY], posX)
			)
...
function TList:paint()
	...
	for i = 1, self.H - 2 * b do
		...
		gpu.set(
			...
			self.Y + i + b - 1,
			padRight(
				sub(self.lines[i + self.shift] or "", 1, self.W - 2 * b),
				self.W - 2 * b
			)
		)
		if i + self.shift == self.index then
...
function TList:insert(pos, line, item)
	...
	if pos < self.shift + self.H - 1 then
		self:redraw()
	end
end

function TList:sort(comp)
	comp = comp or function(list, i, j)
		return list.lines[j] < list.lines[i]
	end
...
function forms.run(form)
	...
	while work do
		...
		if mouseEv[ev] and adr == gpu.getScreen() then
			activeForm:mouseEv(ev, x, y, btn, user)
		end
		if listeners[ev] then
			for i = 1, #listeners[ev] do
				listeners[ev][i](ev, adr, x, y, btn, user)
			end
		end
		if listeners[""] then
			for i = 1, #listeners[""] do
				listeners[""][i](ev, adr, x, y, btn, user)
			end
		end
	end
